...
### How do I update DClaw Assets?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp assets --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Assets without Kubernetes?

Yes, for local development:

```bash
cd dclaw-assets/frontend && npm run dev
cd dclaw-assets/backend && uvicorn dclaw_assets.main:app --reload
```
