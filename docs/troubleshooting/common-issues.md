...
# Check events
kubectl get events -n dclaw-assets --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-assets deployment/dclaw-assets-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-assets-db-credentials -n dclaw-assets
   ```
